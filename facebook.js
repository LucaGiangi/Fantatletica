  // controlla se connesso per auto-login
  ...
  function newUser(response) {
    ...
    if (response.status === 'connected') {
	  ...
	  FB.api('/me', function(response) {
		  $('#sinistra').load('/wp-content/themes/Iris/profile.php', {FBid: response.id , FBemail: response.email, FBfname: response.first_name, FBlname: response.last_name , FBbirthday: response.birthday , action: 'facebook' });
  	  });
    } else if (response.status === 'not_authorized') {
      // The person is logged into Facebook, but not your app.
