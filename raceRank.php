...
// versione 5 20/07/2015  la classifica viene generata automaticamente dal sistema
...
print'
	...
	<table  class="tab_el schedina">
		<tr>
			...
			<th width="15%" class="td_fantasma" >Costo</th>
			<th width="15%" class="td_fantasma">Numero atleti</th>
		</tr>';    
  
$stringa_rank=Rank(NULL,$_GET['id']); // leggi punteggi atleti
...
foreach($json_o as $p){
	...
	for ($i=0;$i< sizeof($p[Rank]);$i++){
		...
		if ($i>0){
			if (!($p[Rank][$i-1][Punti]==$p[Rank][$i][Punti] && $p[Rank][$i][Costo]==$p[Rank][$i-1][Costo] && $p[Rank][$i][Numero]==$p[Rank][$i-1][Numero]))			
			$j++;
		}else
			$j++;
		...
		if ($j==0) print '<img  class="gold" src="images/gold.png">'; 
		if ($j==1) print '<img  class="silver" src="images/silver.png">';
		if ($j==2)  print '<img  class="silver" src="images/bronze.png">';// print '<img  class="bronze" src="images/bronze.png">';
		
		if ($j>2) print ($j+1)."&deg;"; 
		print'</td><td>'.$p[Rank][$i][Nome].'</td>';
		print '<td style="text-align:center">'.$p[Rank][$i][Punti].'</td>';
		print '<td class="td_fantasma" style="text-align:center">'.$p[Rank][$i][Costo].'</td>';
		print '<td class="td_fantasma" style="text-align:center">'.$p[Rank][$i][Numero].'</td>';
		print'</tr>';	
		//$i++;	
