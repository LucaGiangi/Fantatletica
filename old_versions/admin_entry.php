...
//**************************
$id_competizione=7;
$q="SELECT * FROM Competizione Where Id_competizione=$id_competizione";					
$t=mysql_query($q,$connessione); 	
	while($r=mysql_fetch_object($t)){
		$Competizione_nome=$r->Nome;
		$Competizione_sede=$r->Sede;
	}
?>
<!doctype html>
<html lang="it-IT">
...
<title>AtletiperCaso-Il gioco</title>
</head>
...
/* versione 3_0 *** LA VERIFICA DI UNA GARA ATTIVA VIENE FATTA AL MOMENTO DEL LOGIN */
	...
	if ($id_competizione!=-1){
		print'<div class="titolo"><hh2>'.$Competizione_nome.'<br/>pagina test iscirzioni</hh2></div>';

	   print' <div class="gara">
        	<div class="gara_contenuto">';
			if ($cont!=0) print ' <hr>';
            	print '<div class="ab_sx">
				<h2>Sede: '.$Competizione_sede.'</h2>
  ...
  <!--   <div id="prova">prova</div> -->
      <!-- ** -->
        ...
        <div>
   <?php //	if ($id_competizione!=-1) print'<articolo>In questa lista troverai gli atleti iscritti alla gara.<br/>Clicca sul nome dell&acute;atleta che vuoi nella tua squadra ed entrerà a far parte del tuo team.</articolo>';?>
<p></p>
        </div>
          <!-- *************  -->      
...
if ($completa== false){
		...
		if ($id_competizione!=-1){//verifica presenza di competizioni attive
					...
					// mostra iscritti alla gara		  
								...
								$gara_prec = "";	
								$id_table=0;			
								$q="SELECT * FROM Iscritti JOIN Atleti ON Iscritti.Atleti_id_atleti = Atleti.Id_Atleta where Competizione_Id_competizione='".$id_competizione."' ORDER BY Atleti.sesso desc, Iscritti.Gara_Gara1,Atleti.Cognome";
								$t=mysql_query($q,$connessione); 	
								while($r=mysql_fetch_object($t)){
	...
	print '<table width="100%" class="tab_el" id='.$id_table.'> 
			...
			$q="SELECT * FROM Team 
						...
						JOIN Atleti ON Iscritti.Atleti_id_atleti = Atleti.Id_Atleta where Team.Giocatore_Id_giocatore='".$g_id/*$_SESSION['g_id']*/."' order by Capitano";
			$t=mysql_query($q,$connessione); 	
			while($r=mysql_fetch_object($t)){
				
				 if ($riga){print '<tr class="pari">';} else {print '<tr class="dispari">';}	
												print'<td><img src="images/Flag_of_Italy.svg.png" TITLE="Nazionalit&agrave" width="30"/></td>';	
										  $Avatar = getAvatarByName($r->Nome,$r->Cognome); 
									/*	  if ($r->Avatar=="") 
											$Avatar = "images/profilo.jpg";
										  else
											$Avatar = "images/atleti/".$r->Avatar;*/
											
										  print'<td><img class="lazy" src="images/default_avatar.png" data-original="'.$Avatar.'" width="30" title="Profilo" /></td>';
...
xmlhttp.open("GET","php/print_gara.php?gara="+div+"<?php print '&id='.$id_competizione; ?>",true);
xmlhttp.send(null);
}
