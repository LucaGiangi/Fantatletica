<?php
$page_name = "Read_schedina_giocatori";
...
$riga=false;
$i=1;
$q="SELECT * FROM Giocate JOIN Giocatore on Giocatore.Id_giocatore=Giocate.Giocatore_Id_giocatore where Competizione_id_competizione='".$_GET['id']."'";
$t=mysql_query($q,$connessione); 			
while($r=mysql_fetch_object($t)){	
	if ($riga) print '<tr class="pari">'; else print '<tr class="dispari">';
	$riga= !$riga;
	
	print '<td>'.$i.'</td><td>'.$r->Nome_team.'</td><td>'.$r->Puntata.'</td><td>'.$r->Time.'</td><td><a href="Admin_view_schedina_personale.php?id=8&id_giocatore='.$r->Giocatore_Id_giocatore.'" target="new">Guarda</a></td></tr>';
	$i++;	
}
